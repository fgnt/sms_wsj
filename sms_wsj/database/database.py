...
class SmsWsj(lazy_dataset.database.JsonDatabase):
    ...
    def __init__(
            self,
            json_path='/scratch/hpc-prf-nt1/cbj/sms_wsj/sms_wsj.json',
            # json_path='/scratch/hpc-prf-nt1/cbj/tmp/sms_wsj/sms_wsj.json',
            # json_path='/scratch/hpc-prf-nt1/jensheit/jsons/final_sms_wsj.json',
    ):
...
class AudioReader:
    ...
    def _rec_audio_read(cls, file):
        ...
        if isinstance(file, (tuple, list)):
            data = [cls._rec_audio_read(f) for f in file]
            data_np = np.array(data)
            if data_np.dtype == np.object:
                return data
            else:
                return data_np
