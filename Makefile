...
original_sms_wsj.json: $(JSON_DIR)/original_sms_wsj.json
$(JSON_DIR)/original_sms_wsj.json: $(JSON_DIR)/wsj_8k_zeromean.json | $(JSON_DIR) $(RIR_DIR)
	@echo creating $(JSON_DIR)/org_sms_wsj.json
	python -m sms_wsj.database.create_original_json \
...
$(SMS_WSJ_DIR)/observation: $(JSON_DIR)/original_sms_wsj.json | $(SMS_WSJ_DIR)
	...
	with dst_dir=$(SMS_WSJ_DIR) json_path=$(JSON_DIR)/original_sms_wsj.json write_all=$(WRITE_ALL) debug=$(DEBUG)

sms_wsj.json: $(JSON_DIR)/sms_wsj.json
$(JSON_DIR)/sms_wsj.json: $(JSON_DIR)/original_sms_wsj.json | $(SMS_WSJ_DIR)
	...
	with db_dir=$(SMS_WSJ_DIR) original_json_path=$(JSON_DIR)/original_sms_wsj.json write_all=$(WRITE_ALL) json_path=$(JSON_DIR)/sms_wsj.json debug=$(DEBUG)
