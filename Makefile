...
$(JSON_DIR)/sms_wsj.json: $(RIR_DIR) $(JSON_DIR) $(JSON_DIR)/wsj_8k_zeromean.json
	@echo creating $(JSON_DIR)/sms_wsj.json
	python -m sms_wsj.database.create_json \
	with json_path=$(JSON_DIR)/sms_wsj.json rir_dir=$(RIR_DIR) wsj_json_path=$(JSON_DIR)/wsj_8k_zeromean.json
...
$(SMS_WSJ_DIR)/sms_wsj: $(JSON_DIR)/sms_wsj.json $(SMS_WSJ_DIR)
	...
	with dst_dir=$(SMS_WSJ_DIR) json_path=$(JSON_DIR)/sms_wsj.json write_all=$(WRITE_ALL) new_json_path=$(JSON_DIR)/sms_wsj.json
